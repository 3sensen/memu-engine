    ...
    async def embed(self, inputs: list[str]) -> tuple[list[list[float]], CreateEmbeddingResponse | None]:
        ...
        if len(inputs) <= self.embed_batch_size:
            ...
            try:
                ...
                logger.info("LLM-embed tokens usage[%s]: %s (input: %s, output: %s)", ms, getattr(response.usage, "total_tokens", "N/A"), getattr(response.usage, "prompt_tokens", "N/A"), getattr(response.usage, "completion_tokens", "N/A"))
            except Exception as e:
